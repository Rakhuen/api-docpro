...
```javascript
  ...
  //fileImage diambil dari onChange <input type="file" onChange={(e) => setFileImage(e.target.files[0])} />
  //jangan lupa consolo log fileImage supaya tau isi dari fileImage
  //ini hanya sekedar opsi, atau bisa dicari lebih di google atau stackoverflow
